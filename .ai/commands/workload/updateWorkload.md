...
### 2. Template Files (`Workload/Manifest/`) - Rarely Edited

**⚠️ Important**: These templates use placeholder tokens like `{{WORKLOAD_NAME}}` and are **only edited when changing the manifest structure itself**, not for configuration values.

For configuration changes (workload name, version, URLs, etc.), edit `.env` files instead. Templates are automatically processed during builds.

#### `WorkloadManifest.xml` Template
...
### 3. Generated Files (`build/`) - Never Edit Manually

**⚠️ Auto-Generated**: Files in `build/Manifest/` and `build/DevGateway/` are automatically created by running `npm run build:dev/test/prod`. Changes made directly to these files will be overwritten.

To update generated files, modify the source `.env` files and rebuild.
...
Regenerate all files in `build/` with the updated workload name:
...
1. **Run Build Script**:
   ...
   This regenerates:
   - `build/Manifest/WorkloadManifest.xml` with actual workload name
   - `build/Manifest/*Item.xml` files
...
- [ ] `.env` files updated with new `WORKLOAD_NAME` value
- [ ] Build regenerated with `npm run build:dev` (or build:test/prod)
- [ ] Generated `build/Manifest/WorkloadManifest.xml` contains correct workload name (verify, don't edit)
- [ ] All generated `build/Manifest/*Item.xml` files reference correct workload (verify, don't edit)
- [ ] Template files still use `{{WORKLOAD_NAME}}` placeholders correctly
...
- Verify `.env` files contain correct `WORKLOAD_NAME`
- Check generated `build/Manifest/WorkloadManifest.xml` has correct `WorkloadName` (verify only, don't edit)
- Rebuild manifest package with `npm run build:dev`
- Restart dev gateway
